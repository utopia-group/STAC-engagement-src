...
public class SparseLongLongArray {
    ...
    private long get(long key, long valueIfKeyNotFound) {
        int i = SparseLongLongArray.binarySearch(this.mKeys, 0, this.mSize, key);
    ...
    public int put(long key, long value) {
        ...
        if (i >= 0) {
            ...
            if ((i ^= -1) < this.mSize && this.mValues[i] == Long.MIN_VALUE) {
                this.mKeys[i] = key;
                this.mValues[i] = value;
                return i;
            }
            if (this.mGarbage && this.mSize >= this.mKeys.length) {
                this.gc();
                i = ~ SparseLongLongArray.binarySearch(this.mKeys, 0, this.mSize, key);
            }
    ...
    private int indexOfKey(long key) {
        ...
        return SparseLongLongArray.binarySearch(this.mKeys, 0, this.mSize, key);
    }
