...
public class TestAlgoCollector {
    ...
    public TestAlgoCollector assertDistance(AlgoHelperEntry algoEntry, List<QueryResult> queryList, OneRun oneRun) {
        ArrayList<Path> viaPaths = new ArrayList<Path>();
        ...
        for (int i = 0; i < queryList.size() - 1; ++i) {
            Path path = algoEntry.createAlgo(queryGraph).calcPath(queryList.get(i).getClosestNode(), queryList.get(i + 1).getClosestNode());
            viaPaths.add(path);
        }
        PathMerger pathMerger = new PathMerger().setCalcPoints(true).setSimplifyResponse(false).setEnableInstructions(true);
        GHResponse rsp = new GHResponse();
        pathMerger.doWork(rsp, viaPaths, this.trMap.getWithFallBack(Locale.US));
        if (rsp.hasErrors()) {
            this.errors.add(algoEntry + " response contains errors. Expected distance: " + rsp.getDistance() + ", expected points: " + oneRun + ". " + queryList + ", errors:" + rsp.getErrors());
            return this;
        }
