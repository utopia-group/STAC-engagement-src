...
extends SymmetricCipher {
    ...
    private static long getLongFromBytes(byte[] ba, int offset) {
        long l = 0;
        for (int i = 0; i < 8; ++i) {
            byte value = offset + i < ba.length ? ba[offset + i] : 0;
    ...
    private static void getBytesFromLong(byte[] ba, int offset, long l) {
        for (int i = 7; i >= 0 && offset + i < ba.length; --i) {
    ...
    private static long encryptBlock(long m, long key) {
        long[] subkeys = DES.createSubkeys(key);
        long ip = DES.IP(m);
        int l = (int)(ip >> 32);
    ...
    private static void encryptBlock(byte[] message, int messageOffset, byte[] ciphertext, int ciphertextOffset, byte[] key) {
        ...
        DES.getBytesFromLong(ciphertext, ciphertextOffset, c);
    }

    private static byte[] encrypt(byte[] message, byte[] key, byte[] ciphertext) {
    ...
    private static byte[] extractKey(byte[] pwbytes) {
        byte[] key = new byte[8];
        for (int i = 0; i < 8; ++i) {
            if (i < pwbytes.length) {
                byte b = pwbytes[i];
    ...
    private static byte[] extractBlock(byte[] block) {
        if (block.length == 8) {
            return block;
        }
    ...
    @Override
    public void encrypt_ctr(OpenSSLRSAPEM.INTEGER counter, byte[] key, ByteArrayInputStream is, ByteArrayOutputStream os) throws IOException {
        byte[] temp = new byte[8];
        byte[] cipher = new byte[8];
        while (is.available() > 0) {
            DES.encrypt(DES.extractBlock(counter.getBytes()), DES.extractKey(key), cipher);
            int read = is.read(temp);
            for (int i = 0; i < read; ++i) {
                os.write(temp[i] ^ cipher[i]);
            }
        }
    }
}
