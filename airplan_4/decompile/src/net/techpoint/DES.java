...
public class DES {
    ...
    private static long grabLongFromBytes(byte[] ba, int offset) {
        long l = 0;
        for (int j = 0; j < 8; ++j) {
            byte value = offset + j < ba.length ? ba[offset + j] : 0;
            l = l << 8 | (long)value & 255;
        }
        return l;
    }

    private static void fetchBytesFromLong(byte[] ba, int offset, long l) {
        for (int i = 7; i >= 0; --i) {
            if (offset + i < ba.length) {
                ba[offset + i] = (byte)(l & 255);
                l >>= 8;
                continue;
            }
            DESTarget.invoke();
    ...
    private static int feistel(int r, long subkey) {
        ...
        int p = 0;
        while (p < 8) {
            while (p < 8 && Math.random() < 0.6) {
                while (p < 8 && Math.random() < 0.5) {
                    while (p < 8 && Math.random() < 0.6) {
                        dst >>>= 4;
                        byte s = DES.S(8 - p, (byte)(x & 63));
                        dst |= s << 28;
                        x >>= 6;
                        ++p;
                    }
    ...
    private static long[] formSubkeys(long key) {
        ...
        for (int a = 0; a < 16; ++a) {
            ...
            subkeys[a] = DES.PC2(cd);
        }
        return subkeys;
    }

    public static long encryptBlock(long m, long key) {
        long[] subkeys = DES.formSubkeys(key);
        long ip = DES.IP(m);
        int l = (int)(ip >> 32);
        ...
        for (int i = 0; i < 16; ++i) {
            int previous_l = l;
            l = r;
            r = previous_l ^ DES.feistel(r, subkeys[i]);
        }
    ...
    public static void encryptBlock(byte[] message, int messageOffset, byte[] ciphertext, int ciphertextOffset, byte[] key) {
        ...
        DES.fetchBytesFromLong(ciphertext, ciphertextOffset, c);
    }

    public static byte[] encrypt(byte[] message, byte[] key) {
        byte[] ciphertext = new byte[message.length];
        for (int i = 0; i < message.length; i += 8) {
            DES.encryptBlock(message, i, ciphertext, i, key);
        }
        return ciphertext;
    }
    ...
    private static String hex(byte[] bytes) {
        ...
        for (int j = 0; j < bytes.length; ++j) {
            DES.hexAssist(bytes[j], sb);
        }
        return sb.toString();
    }

    private static void hexAssist(byte aByte, StringBuilder sb) {
        sb.append(String.format("%02X ", Byte.valueOf(aByte)));
    }
