...
public class Instruction {
    ...
    long fillGPXList(List<GPXEntry> list, long time, Instruction prevInstr, Instruction nextInstr, boolean firstInstr) {
        this.checkOne();
        ...
        for (int i = 0; i < len; ++i) {
            ...
            double d = nextLon = last ? nextInstr.getFirstLon() : this.points.getLongitude(i + 1);
            double nextEle = is3D ? (last ? nextInstr.getFirstEle() : this.points.getElevation(i + 1)) : Double.NaN;
            prevTime = is3D ? Math.round((double)prevTime + (double)this.time * Helper.DIST_3D.calcDist(nextLat, nextLon, nextEle, lat, lon, ele) / this.distance) : Math.round((double)prevTime + (double)this.time * Helper.DIST_3D.calcDist(nextLat, nextLon, lat, lon) / this.distance);
            lat = nextLat;
            lon = nextLon;
            ele = nextEle;
        }
    ...
    public double calcAzimuth(Instruction nextI) {
        ...
        } else if (nextI != null && this.points.getSize() == 1) {
