...
public class ConsistentWeightApproximator {
    ...
    public double approximate(int fromNode, boolean reverse) {
        double weightApproximation = 0.5 * (this.uniDirApproximatorForward.approximate(fromNode) - this.uniDirApproximatorReverse.approximate(fromNode));
        if (reverse) {
            weightApproximation *= -1.0;
        }
