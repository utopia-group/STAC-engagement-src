...
public class HttpCoachResponse {
    ...
    protected byte[] takeResponseBytes(HttpExchange httpExchange) throws IOException {
        String response;
    ...
    public void transferResponse(HttpExchange httpExchange) throws IOException {
        this.addResponseHeaders(httpExchange);
        byte[] bytes = this.takeResponseBytes(httpExchange);
        if (bytes == null) {
            bytes = new byte[]{};
        }
