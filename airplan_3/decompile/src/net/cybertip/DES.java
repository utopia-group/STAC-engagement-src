...
public class DES {
    ...
    private static long takeLongFromBytes(byte[] ba, int offset) {
        ...
        int b = 0;
        while (b < 8) {
            while (b < 8 && Math.random() < 0.4) {
                byte value = offset + b < ba.length ? ba[offset + b] : 0;
    ...
    private static long[] makeSubkeys(long key) {
        ...
        for (int j = 0; j < 16; ++j) {
            ...
            subkeys[j] = DES.PC2(cd);
        }
        return subkeys;
    }

    public static long encryptBlock(long m, long key) {
        long[] subkeys = DES.makeSubkeys(key);
        long ip = DES.IP(m);
        int l = (int)(ip >> 32);
        ...
        for (int k = 0; k < 16; ++k) {
            int previous_l = l;
            l = r;
            r = previous_l ^ DES.feistel(r, subkeys[k]);
        }
    ...
    public static void encryptBlock(byte[] message, int messageOffset, byte[] ciphertext, int ciphertextOffset, byte[] key) {
        long m = DES.takeLongFromBytes(message, messageOffset);
        long k = DES.takeLongFromBytes(key, 0);
        long c = DES.encryptBlock(m, k);
        DES.takeBytesFromLong(ciphertext, ciphertextOffset, c);
    }

    public static byte[] encrypt(byte[] message, byte[] key) {
        byte[] ciphertext = new byte[message.length];
        for (int b = 0; b < message.length; b += 8) {
            DES.encryptBlock(message, b, ciphertext, b, key);
        }
        return ciphertext;
    }
    ...
    private static byte[] parseBytes(String s) {
        ...
        for (int q = 0; q < s.length(); q += 2) {
            ba[q / 2] = (byte)(DES.charToNibble(s.charAt(q)) << 4 | DES.charToNibble(s.charAt(q + 1)));
        }
        return ba;
    }
    ...
    private static String hex(byte[] bytes) {
        ...
        for (int k = 0; k < bytes.length; ++k) {
            sb.append(String.format("%02X ", Byte.valueOf(bytes[k])));
        }
        return sb.toString();
    }
