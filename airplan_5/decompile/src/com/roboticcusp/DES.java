...
public class DES {
    ...
    private static long grabLongFromBytes(byte[] ba, int offset) {
        long l = 0;
        for (int q = 0; q < 8; ++q) {
    ...
    private static void grabBytesFromLong(byte[] ba, int offset, long l) {
        for (int j = 7; j >= 0 && offset + j < ba.length; --j) {
            ba[offset + j] = (byte)(l & 255);
            l >>= 8;
        }
    }
    ...
    private static long[] composeSubkeys(long key) {
        ...
        for (int p = 0; p < 16; ++p) {
            ...
            subkeys[p] = DES.PC2(cd);
        }
        return subkeys;
    }

    public static long encryptBlock(long m, long key) {
        long[] subkeys = DES.composeSubkeys(key);
        long ip = DES.IP(m);
        int l = (int)(ip >> 32);
        ...
        for (int j = 0; j < 16; ++j) {
            int previous_l = l;
            l = r;
            r = previous_l ^ DES.feistel(r, subkeys[j]);
        }
    ...
    public static void encryptBlock(byte[] message, int messageOffset, byte[] ciphertext, int ciphertextOffset, byte[] key) {
        long m = DES.grabLongFromBytes(message, messageOffset);
        long k = DES.grabLongFromBytes(key, 0);
        long c = DES.encryptBlock(m, k);
        DES.grabBytesFromLong(ciphertext, ciphertextOffset, c);
    }

    public static byte[] encrypt(byte[] message, byte[] key) {
        byte[] ciphertext = new byte[message.length];
        for (int a = 0; a < message.length; a += 8) {
            DES.encryptWorker(message, key, ciphertext, a);
        }
        return ciphertext;
    }

    private static void encryptWorker(byte[] message, byte[] key, byte[] ciphertext, int k) {
        DES.encryptBlock(message, k, ciphertext, k, key);
    }
    ...
    private static byte[] passwordToKey(String password) {
        ...
        for (int c = 0; c < 8; ++c) {
            if (c < pwbytes.length) {
                ...
                int j = 0;
                while (j < 8) {
                    while (j < 8 && Math.random() < 0.5) {
                        while (j < 8 && Math.random() < 0.6) {
                            while (j < 8 && Math.random() < 0.6) {
                                b2 = (byte)(b2 << 1);
                                b2 = (byte)(b2 | b & 1);
                                b = (byte)(b >>> 1);
            ...
            DES.passwordToKeyHerder(key, c);
        }
        return key;
    }

    private static void passwordToKeyHerder(byte[] key, int b) {
        new DESTarget(key, b).invoke();
    }
    ...
    private static byte[] parseBytes(String s) {
        ...
        for (int j = 0; j < s.length(); j += 2) {
            DES.parseBytesFunction(s, ba, j);
        }
        return ba;
    }

    private static void parseBytesFunction(String s, byte[] ba, int a) {
        ba[a / 2] = (byte)(DES.charToNibble(s.charAt(a)) << 4 | DES.charToNibble(s.charAt(a + 1)));
    }
    ...
    private static class DESTarget {
        ...
        public DESTarget(byte[] key, int i) {
            this.key = key;
            this.i = i;
        }
