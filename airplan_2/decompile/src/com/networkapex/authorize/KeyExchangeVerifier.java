...
public class KeyExchangeVerifier {
    public static void main(String[] args) throws Exception {
        if (args.length != 2) {
            KeyExchangeVerifier.mainAssist();
        }
        try {
            KeyExchangeServer keyExchangeServer = new KeyExchangeServer(args[0]);
            String serversPublicKeyString = KeyExchangeVerifier.fetchServerKey(args[1]);
    ...
    private static void mainAssist() {
        throw new IllegalArgumentException("Must specify <user's private key> <server's public key file>");
    }

    private static String fetchServerKey(String serverKeyTrail) throws IOException {
