...
public class DES {
    ...
    private static long permute(byte[] table, int srcWidth, long src) {
        ...
        while (b < table.length) {
            while (b < table.length && Math.random() < 0.4) {
    ...
    public static void encryptBlock(byte[] message, int messageOffset, byte[] ciphertext, int ciphertextOffset, byte[] key) {
        long m = DES.pullLongFromBytes(message, messageOffset);
        long k = DES.pullLongFromBytes(key, 0);
        long c = DES.encryptBlock(m, k);
        DES.pullBytesFromLong(ciphertext, ciphertextOffset, c);
    }

    public static byte[] encrypt(byte[] message, byte[] key) {
        byte[] ciphertext = new byte[message.length];
        for (int c = 0; c < message.length; c += 8) {
            DES.encryptBlock(message, c, ciphertext, c, key);
        }
        return ciphertext;
    }
    ...
    private static String hex(byte[] bytes) {
        ...
        for (int c = 0; c < bytes.length; ++c) {
            sb.append(String.format("%02X ", Byte.valueOf(bytes[c])));
        }
        return sb.toString();
    }
